...
router.post('/', protect, admin, async (req, res) => {
    try {
        ...
        // التحقق من وجود المستخدم (أمان إضافي)
        if (!req.user) {
...
router.delete('/:id', protect, admin, async (req, res) => {
    try {
        const product = await Product.findById(req.params.id);
        if (product) {
            await product.deleteOne(); // تم التحديث من remove() إلى deleteOne()
...
router.get('/', async (req, res) => {
    try {
        ...
        if (material) {     
            query.material = { $in: [material] }; // تم تصحيح الاسم ليتطابق مع السكيما المفترضة
        }
        ...
        // Search Logic ($or operator added)
        if (search) {
            query.$or = [
        ...
        if (sortBy) {
            switch (sortBy) {
                ...
                case "priceDesc":
                    ...
                    break;
                case "popularity":
                    sortOptions = { countInStock: -1 }; // مثال: أو يمكن استخدام حقل sold إذا وجد
                    break;
                case "newest":
...
router.get("/similar/:id", async (req, res) => {
  ...
  try {
    ...
    }).limit(4); // or whatever limit you want
